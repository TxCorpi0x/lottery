...
import { MsgCreateBet } from "./types/lottery/bet/tx";
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgCreateBet({ value }: msgCreateBetParams): EncodeObject {
			try {
				return { typeUrl: "/vjdmhd.lottery.bet.MsgCreateBet", value: MsgCreateBet.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			VjdmhdLotteryBet: new SDKModule(test)
		},
		registry: msgTypes
  }
}
